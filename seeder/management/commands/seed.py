        ...
        if confirmation:
            ...
            Task.objects.create(
                ...
                description="The Uncomplicated Firewall (UFW) is a user-friendly front-end for managing iptables, the "
                            ...
                            "this guide, you will secure a Linux system with the UFW and learn how to easily block "
                            "traffic on certain ports.\n\n1. Update the package repositories\nKali maintains a list of "
                            "packages (i.e. programs). These packages can be installed using the package manager APT. "
                            "To ensure that you install the latest version of the UFW, please update the package "
                            "repositories using:\n\napt update\n\n2. Install the UFW\n\napt install -y ufw\n\n3. "
                            "Setting up default policies\nIf you are setting up a firewall, the rules you will most "
                            "likely begin with are the default rules. They will be followed if no other, more specific "
                            "rule applies. Usually, it is a good idea to deny every incoming traffic (meaning every "
                            "traffic from the outside cannot reach the server) and allow every outgoing traffic "
                            "(meaning services running on the server itself can still reach the outside network). "
                            "Remember: There will be more specific rules that are exceptions to this, but as a "
                            "default, this is a good starting point. To set up these rules, type in:\n\nufw default "
                            "deny incoming\nufw default allow outgoing\n\n4. Allow certain services:\nIf you were to "
                            "enable the firewall now, no traffic from outside could reach the system, including "
                            "connections like SSH. For this guide, assume your system runs an SSH server. "
                            "Consequently, it makes sense to allow SSH connections:\n\nufw allow ssh\n\nCheck the "
                            "changes you have made by running:\n\nufw status\n\nFor a real-world example, imagine you "
                            "are running a web server on port 80 (HTTP) and 443 (HTTPS). To allow traffic to reach the "
