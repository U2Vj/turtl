...
enrollmentStore.getEnrollment(props.enrollmentId).then((enrollment) => {
...
function submitSolution() {
  ...
  task.value.acceptance_criteria.flags?.filter(
      flag => !flag.solution || flag.solution.trim().length === 0
  ).forEach(flag => errorMessages.value.flagErrors.push({
    id: flag.id,
    error: 'You must enter a solution.'
  }))
  task.value.acceptance_criteria.regexes?.filter(
      regex => !regex.solution || regex.solution.trim().length === 0
  ).forEach(regex => errorMessages.value.regExErrors.push({
    id: regex.id,
    error: 'You must enter a solution.'
  }))

  // check if some questions have no answers checked
  task.value.acceptance_criteria.questions?.filter(
      question => !question.choices.some(choice => choice.checked)
  ).forEach(question => errorMessages.value.questionErrors.push({
    id: question.id,
  ...
  enrollmentStore.submitSolution(task.value).then((response) => {
  ...
  <template v-if="task">
    ...
    <v-main class="mb-4">
      <v-container fluid>
        ...
        <v-row>
          <v-col cols="6">
            <p style="white-space: pre-wrap">{{ task.description }}</p><br>
          </v-col>
          <v-col cols="6">
            <Shell />
          </v-col>
      ...
      <v-container>
        <v-form @submit.prevent="submitSolution">
          ...
          <v-divider class="mt-4 mb-4" v-if="task.acceptance_criteria.flags && task.acceptance_criteria.flags.length > 0" />
          ...
          <v-divider v-if="task.acceptance_criteria.regexes && task.acceptance_criteria.regexes.length > 0" class="mt-4 mb-4" />
                ...
                <v-card-text>
                  <div v-for="choice in question.choices">
                    <v-radio-group
                      ...
                      @update:model-value="choiceId => setSingleChoiceSelectedAnswerOption(question.id, choiceId)"
                    >
          ...
          <v-divider v-if="task.acceptance_criteria.questions && task.acceptance_criteria.questions.length > 0" class="mt-4 mb-4"/>
          <v-row v-if="task.done">
            <v-col>
              <v-sheet color="cardColor" class="pa-4">
                <p>
                  <strong>Please note:</strong> You have already solved this task. You can try again and check if the values you entered are
                  correct, but your initial solution will remain and will not be lost or changed.
                </p>
              </v-sheet>
            </v-col>
          </v-row>
